...
上下文模式 (context_mode) - 强烈推荐使用 "group"：
\u2022 "group"（默认选项）：任务将正常顺延当前群组的对话上下文运行，可以访问聊天历史与环境感知。只要用户未在指令中明确提出“隔离”、“沙盒”或“后台静默”等要求，**永远**优先使用这个模式。
\u2022 "isolated"：任务将在一个没有任何上下文的纯净隔离沙盒中运行。由于会丧失用户交互历史记忆，**除非**用户明确指令（例如“建立一个独立的后台脚本”、“纯粹的数据爬取作业”），否则**严禁**主动猜测并使用该模式。

如果不确定，一律退回到 group 模式来保证响应连贯性。
