...
digraph skill_flow {
    ...
    "准备调用 EnterPlanMode 工具进入计划模式？" [shape=doublecircle];
    ...
    "调用 TodoWrite 工具为每一项创建任务" [shape=box];
    "严格遵循 Skill 指引" [shape=box];
    "做出回复 (包括澄清)" [shape=doublecircle];

    "准备调用 EnterPlanMode 工具进入计划模式？" -> "已经过头脑风暴？";
    ...
    "包含清单 (Checklist) 吗？" -> "调用 TodoWrite 工具为每一项创建任务" [label="是"];
    "包含清单 (Checklist) 吗？" -> "严格遵循 Skill 指引" [label="否"];
    "调用 TodoWrite 工具为每一项创建任务" -> "严格遵循 Skill 指引";
}
```
