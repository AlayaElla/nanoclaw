...
const DECISION_PATTERNS: RegExp[] = [
  ...
  // Chinese patterns
  /(不要|不用|取消|放弃|不能用|别用|换成|改为|用.*代替|代替|更换|换一个|思路|换种|尝试|选|选这个|不如用|不再使用|替换为|就用)/,
];
...
const ROLE_PATTERNS: RegExp[] = [
  ...
  // Chinese patterns
  /(你现在是|充当|扮演|作为|身份是|你是|模拟|人设是|设定是|担任|资深|高级工程师|技术专家|架构师)/,
];
...
const INTENT_PATTERNS: Array<{ mode: string; pattern: RegExp }> = [
  ...
  // Chinese patterns
  { mode: "investigate", pattern: /(为什么|怎么做|原理解释|机制|分析|调试报错|找bug|调查|看看|排查|查一下|找一下)/ },
  { mode: "implement",   pattern: /(创建|添加|加上|构建|写一下|写一段|写一个|开发|实现|修复|修一下|解决这个bug|完成)/ },
  { mode: "discuss",     pattern: /(考虑|你觉得|能不能|如何看待|有什么建议|怎么看|方案对比|优缺点|评价一下|讨论|想一想)/ },
  { mode: "review",      pattern: /(审查|检查|查漏补缺|审计|验证|测试|跑测|校验一下|帮我看看代码)/ },
];
