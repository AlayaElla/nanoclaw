...
export function renderData(dataEvents: StoredEvent[]): string {
  if (dataEvents.length === 0) return "";
  const lines = ["  <data_references>"];
  for (const ev of dataEvents) {
    lines.push(`    - ${escapeXML(truncateString(ev.data, 300))}`);
  }
  lines.push("  </data_references>");
  return lines.join("\n");
}
...
export function buildResumeSnapshot(
  ...
  const roles = renderRoles(roleEvents);
  if (roles) sections.push(roles);

  const dataRefs = renderData(dataEvents);
  if (dataRefs) sections.push(dataRefs);
