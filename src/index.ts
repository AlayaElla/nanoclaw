...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const onIpcStatus = async (event: IpcStatusEvent) => {
    ...
    if (event.status === 'running' && event.tool) {
      // Telegram draft bubbles are intentionally allowed to be erased natively
      // by the tool status message. We will recreate the draft bubble seamlessly
      // on the next streamed token since Telegram API 9.3 supports it!
