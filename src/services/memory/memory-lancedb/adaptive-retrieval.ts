...
const SKIP_PATTERNS = [
  ...
  /^(go ahead|continue|proceed|do it|start|begin|next|实施|实拖|开始|開始|继续|繼續|好的|可以|行)\s*[.!]?$/i,
  // Pure emoji
  /^[\p{Emoji}\s]+$/u,
...
function normalizeQuery(query: string): string {
  ...
  const metadataPattern =
    /^(Conversation info|Sender) \(untrusted metadata\):[\s\S]*?\n\s*\n/gim;
  ...
  s = s
    .trim()
    .replace(/^\[[A-Za-z]{3}\s\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}\s[^\]]+\]\s*/, '');
 ...
 */
export function shouldSkipRetrieval(
  query: string,
  minLength?: number,
): boolean {
  ...
  if (
    trimmed.length < defaultMinLength &&
    !trimmed.includes('?') &&
    !trimmed.includes('？')
  )
    return true;
