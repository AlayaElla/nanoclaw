...
export class DecayEngine {
  ...
  private getTierBeta(tier: MemoryTier): number {
    switch (tier) {
      case 'core':
        return this.config.betaCore;
      case 'working':
        return this.config.betaWorking;
      case 'peripheral':
        return this.config.betaPeripheral;
    }
  }
  ...
  private getTierFloor(tier: MemoryTier): number {
    switch (tier) {
      case 'core':
        return this.config.coreDecayFloor;
      case 'working':
        return this.config.workingDecayFloor;
      case 'peripheral':
        return this.config.peripheralDecayFloor;
    }
  }
  ...
  private recency(memory: DecayableMemory, now: number): number {
    ...
    const baseHL =
      memory.temporalType === 'dynamic'
        ? this.config.recencyHalfLifeDays / 3
        : this.config.recencyHalfLifeDays;
    const effectiveHL =
      baseHL * Math.exp(this.config.importanceModulation * memory.importance);
  ...
  public score(memory: DecayableMemory, now: number = Date.now()): DecayScore {
    ...
    const composite =
      this.config.recencyWeight * r +
      this.config.frequencyWeight * f +
      this.config.intrinsicWeight * i;
  ...
  public applySearchBoost(
    results: { memory: DecayableMemory; score: number }[],
    now: number = Date.now(),
  ): void {
    for (const r of results) {
      const ds = this.score(r.memory, now);
      const tierFloor = Math.max(
        this.getTierFloor(r.memory.tier),
        ds.composite,
      );
      const multiplier =
        this.config.searchBoostMin +
        (1 - this.config.searchBoostMin) * tierFloor;
