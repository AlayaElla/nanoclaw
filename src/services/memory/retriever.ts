...
export class MemoryRetriever {
  ...
  public async retrieve(scope: string, query: string, limit: number = 5): Promise<StoreSearchResult[]> {
    ...
    const [vectorResults, bm25Results] = await Promise.all([
      this.store.vectorSearch(scope, queryVector, candidatePoolSize).catch(() => []),
      this.store.bm25Search(scope, query, candidatePoolSize).catch(() => [])
    ]);

    // 3. RRF Fusion
    const fusedResults = await this.fuseResults(scope, vectorResults, bm25Results);
  ...
  private async fuseResults(scope: string, vectorResults: StoreSearchResult[], bm25Results: StoreSearchResult[]): Promise<StoreSearchResult[]> {
    ...
    for (const id of allIds) {
      ...
      const weightedFusion = (vScore * this.config.vectorWeight) + (normalizedBScore * this.config.bm25Weight);
      const fusedScore = clamp01(Math.max(weightedFusion, normalizedBScore >= 0.75 ? normalizedBScore * 0.9 : 0), 0.1);
  ...
  private applyLengthNormalization(results: StoreSearchResult[]): StoreSearchResult[] {
  ...
  private applyMMRDiversity(results: StoreSearchResult[], threshold = 0.85): StoreSearchResult[] {
    ...
    for (const candidate of results) {
      const tooSimilar = selected.some(s => {
        ...
        let sim = 0;
        try { sim = cosineSimilarity(sVec as any, cVec as any); } catch {}
  ...
  private async recordAccess(scope: string, results: StoreSearchResult[]): Promise<void> {
