...
export class TierManager {
  ...
  private workingDemoteThreshold = 0.70;

  public evaluate(memory: DecayableMemory, currentScore: DecayScore, now: number = Date.now()): TierTransition | null {
    ...
    if (memory.tier === 'peripheral' && composite >= this.workingThreshold) {
      if (memory.accessCount >= 2) {
        return { fromTier: 'peripheral', toTier: 'working', reason: 'Score improved and accessed multiple times' };
      }
    }
    ...
    if (memory.tier === 'working' && composite >= this.coreThreshold) {
      if (memory.accessCount >= 5) {
        return { fromTier: 'working', toTier: 'core', reason: 'High score and robust access history' };
      }
    }
    ...
    if (memory.tier === 'core' && composite < this.workingDemoteThreshold) {
      ...
      if (daysSinceAccess > 30) {
        return { fromTier: 'core', toTier: 'working', reason: 'Score degraded and not accessed recently' };
      }
    }
    
    // Demote Working -> Peripheral
    if (memory.tier === 'working' && composite < this.peripheralDemoteThreshold) {
      return { fromTier: 'working', toTier: 'peripheral', reason: 'Score dropped below working threshold' };
    }
