...
async function verify() {
  ...
  try {
    ...
    const textVector = await getEmbedding('Hello from NanoClaw multimodal RAG!');
    console.log('Text vector length:', textVector.length);
    if (textVector.length === 2560) {
      console.log('✅ Text embedding successful (2560 dim)');
    } else {
      console.error('❌ Text embedding failed: expected 2560 dimensions, got', textVector.length);
    }
